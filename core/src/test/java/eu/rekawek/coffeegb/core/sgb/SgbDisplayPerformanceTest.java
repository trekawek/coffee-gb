...
public class SgbDisplayPerformanceTest {
    ...
    @Test
    public void reusesPrimaryGeometryBuffersAndOnlyRebuildsStableBorderOnce() throws IOException {
        ...
        try {
            ...
            assertEquals(1L, display.getFrameArrayAllocations());
            assertEquals(1L, display.getBorderRebuilds());
            assertEquals(2L * Display.DISPLAY_WIDTH * Display.DISPLAY_HEIGHT,
                    display.getCenterPixels());
        } finally {
            eventBus.close();
        }
    }
    ...
    public void retainsSeparateExactGeometryBuffersAndSupportsNestedPublication() throws IOException {
        ...
        try {
            ...
            assertEquals(3L, display.getFrameArrayAllocations());
        } finally {
            eventBus.close();
        }
    }
    ...
    @Test
    public void invalidatesBorderOnlyAtDefinedSourceBoundaries() throws IOException {
        EventBusImpl eventBus = new EventBusImpl(null, "sgb-invalidate", false);
        EventBusImpl sgbBus = new EventBusImpl(null, "sgb-commands", false);
        SgbDisplay display = new SgbDisplay(testRom(), sgbBus, true, true);
        display.init(eventBus);
        try {
            int[] dmg = new int[Display.DISPLAY_WIDTH * Display.DISPLAY_HEIGHT];
            ...
            eventBus.post(new Display.DmgFrameReadyEvent(dmg));
            assertEquals(1L, display.getBorderRebuilds());
            eventBus.post(new Background.SgbBackgroundFadeEvent(0));
            eventBus.post(new Display.DmgFrameReadyEvent(dmg));
            assertEquals(1L, display.getBorderRebuilds());
            eventBus.post(new Background.SgbBackgroundFadeEvent(1));
            eventBus.post(new Display.DmgFrameReadyEvent(dmg));
            assertEquals(2L, display.getBorderRebuilds());
            eventBus.post(new Background.SgbBackgroundReadyEvent(border, mask));
            eventBus.post(new Display.DmgFrameReadyEvent(dmg));
            assertEquals(3L, display.getBorderRebuilds());
            ...
            eventBus.post(new Display.DmgFrameReadyEvent(dmg));
            assertEquals(4L, display.getBorderRebuilds());
        } finally {
            eventBus.close();
            sgbBus.close();
        }

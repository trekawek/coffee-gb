...
public class Mbc2 implements MemoryController {
    ...
    public BessCartridgeState captureBessState() {
        ...
        return new BessCartridgeState(BessCartridgeState.bytes(ram),
                List.of(new MbcWrite(0x0000, ramWriteEnabled ? 0x0a : 0),
                        new MbcWrite(0x2100, selectedRomBank)), Map.of());
    }

    @Override
    public void restoreBessRam(byte[] data) {
        BessCartridgeState.restoreRam(data, ram);
        for (int i = 0; i < ram.length; i++) ram[i] &= 0x0f;
        ramUpdated = true;
    }
    ...
    public ComponentState<MemoryController> captureState() {
        return new Mbc2State(battery.captureState(), ram.clone(), selectedRomBank, ramWriteEnabled, ramUpdated);
    }

    @Override
    public ComponentState<MemoryController> captureState(MachineStateCapture capture) {
        return new Mbc2State(
    ...
    public void restoreState(ComponentState<MemoryController> state) {
        if (!(state instanceof Mbc2State mem)) {
    ...
    private record Mbc2State(ComponentState<Battery> batteryMemento, int[] ram, int selectedRomBank, boolean ramWriteEnabled,

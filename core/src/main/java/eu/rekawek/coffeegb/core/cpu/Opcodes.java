...
import eu.rekawek.coffeegb.core.cpu.opcode.OpcodeBuilder;
...
public final class Opcodes {
    ...
    static {
        ...
        for (Entry<Integer, String> t :
                indexedList(0x04, 0x08, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
            regCmd(opcodes, t, "INC {}").load(t.getValue()).alu("INC").store(t.getValue());
        }

        for (Entry<Integer, String> t :
                indexedList(0x05, 0x08, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
            regCmd(opcodes, t, "DEC {}").load(t.getValue()).alu("DEC").store(t.getValue());
        }

        for (Entry<Integer, String> t :
                indexedList(0x06, 0x08, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
        ...
        for (Entry<Integer, String> t :
                indexedList(0x40, 0x08, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
            for (Entry<Integer, String> s :
                    indexedList(t.getKey(), 0x01, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
                if (s.getKey() == 0x76) {
                    continue;
                }
        ...
        for (Entry<Integer, String> o :
                indexedList(0x80, 0x08, "ADD", "ADC", "SUB", "SBC", "AND", "XOR", "OR", "CP")) {
            for (Entry<Integer, String> t :
                    indexedList(o.getKey(), 0x01, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
                regCmd(opcodes, t, o.getValue() + " {}")
                        .load("A")
                        .alu(o.getValue(), t.getValue())
                        .store("A");
            }
        }
        ...
        for (Entry<Integer, String> o :
                indexedList(0xc6, 0x08, "ADD", "ADC", "SUB", "SBC", "AND", "XOR", "OR", "CP")) {
        ...
        for (Entry<Integer, String> o :
                indexedList(0x00, 0x08, "RLC", "RRC", "RL", "RR", "SLA", "SRA", "SWAP", "SRL")) {
            for (Entry<Integer, String> t :
                    indexedList(o.getKey(), 0x01, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
                regCmd(extOpcodes, t, o.getValue() + " {}")
                        .load(t.getValue())
                        .alu(o.getValue())
                        .store(t.getValue());
            }
        ...
        for (Entry<Integer, String> o : indexedList(0x40, 0x40, "BIT", "RES", "SET")) {
            for (int b = 0; b < 0x08; b++) {
                for (Entry<Integer, String> t :
                        indexedList(o.getKey() + b * 0x08, 0x01, "B", "C", "D", "E", "H", "L", "(HL)", "A")) {
    ...
    private static OpcodeBuilder regLoad(
            OpcodeBuilder[] commands, int opcode, String target, String source) {
    ...
    private static OpcodeBuilder regCmd(
            OpcodeBuilder[] commands, Entry<Integer, String> opcode, String label) {

...
public class ShowBackground implements Command {
  ...
  private static final CommandPattern PATTERN_BACKGROUND =
      CommandPattern.Builder.create("ppu show background")
          .withDescription("display the background tiles")
          .build();

  private static final CommandPattern PATTERN_WINDOW =
      CommandPattern.Builder.create("ppu show window")
  ...
  public void run(ParsedCommandLine commandLine) {
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          JFrame mainWindow =
              new JFrame(type == Type.BACKGROUND ? "Background tiles" : "Window tiles");
  ...
  public class BackgroundTiles extends JPanel implements Runnable {
    ...
    private void drawBackground() {
      ...
      int tileMap =
          type == Type.BACKGROUND ? lcdc.getBgTileMapDisplay() : lcdc.getWindowTileMapDisplay();
          ...
          for (int i = 0; i < 16; i += 2) {
            ...
            if (bank == null) {
              throw new IllegalStateException(
                  "Video bank " + attr.getBank() + " shouldn't be null");
            }

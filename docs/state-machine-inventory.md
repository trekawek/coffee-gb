...
| GPS endpoint | Master ticks, startup beacons, UART output queue/bit delay, RX parser/parity and TAIP command | No time/network/location service is retained. The default core source is deterministic; Android's live location source is runtime-only and is sampled again after restore. |

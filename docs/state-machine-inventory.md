...
the complete replacement and applies the semantic policy registered for each of the 96 admitted
...
The exact field-by-field inventory of all 96 admitted production record types is committed in
...
| Mobile Adapter GB endpoint | Endpoint SB/bit cursor plus engine phase/outcome/error IDs, device ID, packet parser buffer/count/expected size, 256-byte configuration, response and acknowledgement bytes, idle phase, serial-byte observation, and bounded pending-packet slots | The live backend port, sockets, callbacks, and other host resources are never captured. The endpoint record contains only the service-free engine record and reuses the already-configured live backend on restore. |
...
are validated by nested records. `StateInventoryTest` requires the policy-key set to equal all 96

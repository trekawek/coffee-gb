...
- Phase #339 adds no ROM or StateFile fixture. `PackageRuntimeSmoke` constructs a 32 KiB ROM-only
  ...
  infinite two-byte `JR` loop. The headless smoke uses the image only in memory to publish one
  video frame and audio buffer, sample a synthetic A-button press/release, and
  encode/inspect/load/re-encode the existing portable StateFile format. Installed-package CI writes
  the same generated bytes to a create-only file in an isolated `target/` smoke directory, opens
  it through the OS association, and never packages, caches, or uploads it.

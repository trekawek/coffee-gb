...
- Phase #350 adds no fixture or downloaded artifact. PING/PONG, rollback, diagnostics-redaction,
  ...
  rewritten.
- Phase #353 adapter-image import tests create exact 256-byte and 512-byte formula-generated files
  only in private temporary directories. They include boundary, malformed, pathname-replacement,
  null-file-key, and private-store transaction-alias cases but no owner image, account data, ROM,
  save, capture, or downloaded artifact. The format facts are independently restated from pinned
  REON commit `f7bfc0470aed561936b396ed29f2bde50ca601ab`
  `web/htdocs/user/adapter_config.php` (MIT) and pinned libmobile commit
  `0704f56902f23b7ebf05c82c222e0e145e3140b6` `config.c` (LGPL-3.0-or-later); no source text,
  data structure, or control flow is copied, and normal tests perform no network request.
- The owner-authorized Mobile Trainer-to-loopback-REON acceptance probe at Coffee GB revision
  `97fc57b4` adds no committed fixture. One clean session produced two persisted guest configuration
  writes and one browser HTTP request against the pinned loopback REON service. Five bounded
  transfers delivered the declared 19-byte response and ended with remote close; the utility visibly
  rendered `hello world`, backend ownership was already released with clean endpoint parsers, and
  controlled-shutdown cleanup passed.
  The commercial ROM was read in place and left untouched. No ROM, save, private configuration
  image, account or credential value, account identity, local path, checksum, screenshot, screen
  template, raw trace, captured packet payload, or raw captured server-response artifact is
  committed or disclosed. The first-run provider check used an uncommitted loopback prerequisite
  built from the pinned REON POP parser and framing; it is not evidence for production POP, provider
  authentication, accounts, or REON mail-service compatibility. Only the sanitized command/result
  facts, the public HTTP route, and the requested 19-byte declarative hello snippet are retained in
  `docs/mobile-adapter-crystal-reon-validation.md`. Normal tests remain deterministic and offline:
  they neither reproduce the commercial-ROM run nor contact REON.
- Phase #339 adds no committed ROM or StateFile fixture. `PackageRuntimeSmoke` constructs a 32 KiB
  ROM-only DMG image with the title `COFFEE-CI-SMOKE`, a valid reviewable header checksum, and an
  infinite two-byte `JR` loop. The headless smoke places the generated bytes in a private temporary
  7z, verifies byte-exact extraction and archive-entry identity through the isolated helper, and
  deletes all generated files. It then publishes one video frame and audio buffer, samples a
  synthetic A-button press/release, and exercises encode/inspect/load/re-encode with the existing
  portable StateFile format. It reads no external or user ROM and never packages, caches, or
  uploads the generated bytes.

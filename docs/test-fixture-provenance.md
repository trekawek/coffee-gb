...
- The local Phase #353 Mobile Trainer attempt adds no committed fixture. The authorized commercial
  ROM input was read in place and left untouched. Temporary homes/screens/locators, the earlier
  test-only setup-state probe, the later screen-driven state/input tracers, loopback REON checkout,
  generated page file, and captured response artifacts were removed after the probe. Only the
  sanitized command/result boundary and the requested declarative hello snippet are retained in
  `docs/mobile-adapter-crystal-reon-validation.md`; normal tests do not reproduce the
  commercial-ROM run or contact REON.

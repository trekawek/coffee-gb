...
- Phase #339 adds no committed ROM or StateFile fixture. `PackageRuntimeSmoke` constructs a 32 KiB
  ...
  portable StateFile format. Installed-package association CI writes the same generated bytes to a
  create-only file in an isolated `target/` smoke directory and opens it through the OS
  association. Neither path reads an external or user ROM, or packages, caches, or uploads the
  generated bytes.

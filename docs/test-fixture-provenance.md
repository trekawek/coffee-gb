...
Coffee GB keeps generators alongside their test or diagnostic harnesses and commits only the
smallest reviewable expected data:

- `SgbPacketTestBuilder` creates bounded 16-byte packet arrays and JOYP pulses.
- `SgbRendererBaselineTest` generates pixel indices, RGB555 values, tiles, maps, and attributes
  from fixed integer formulas; only expected SHA-256 text is committed.
- `HardwareModelBaselineTest.syntheticRom` creates an in-memory 32 KiB ROM containing an infinite
  two-byte branch, synthetic title/header flags, and a calculated checksum. Its SHA-256 is
  `f89f3802d47dd31da0db6b5656ed5098194e85020ba735fb44c1c9d4f9043eee`.
  SGB state tests use the same generated ROM and production StateFile capture.
- `state-file-v2/sgb2-session-deflate.cgbstate` is generated by Coffee GB from
  `StateCodecTestSupport.rom(seed = 2, sgb = true)`. It contains no Nintendo ROM, BIOS, image, or
  audio bytes; it is 59,486 bytes with SHA-256
  `2d2178e6eba26a8debdacf84be144cccd1b42e50bf0dbce5c41612bcb16aa226`.
- Protocol-v9 TSV vectors use authored synthetic frames, invitation URIs, hashes, errors, and state
  transitions. Tests feed them through the production codec and lifecycle paths. They contain no ROM,
  save, StateFile, real invitation secret, path, credential, endpoint, or peer capture; the public
  invitation token is the explicit test sequence `00..0f`.
- Mobile Adapter commands and transcripts use synthetic packet/configuration values, in-process
  endpoints, and no external network. The protocol facts are restated from pinned Shonumi Dan Docs
  commit `490595c3b8506d3f155aa6be9d7a5cd7d0fa9a5b` (GPL-3.0), with libmobile commit
  `0704f56902f23b7ebf05c82c222e0e145e3140b6` (LGPL-3.0-or-later) as corroboration; no source code,
  data structure, or control flow is copied. Adapter-image import tests create formula-generated
  256-byte and 512-byte files only in private temporary directories.
- `PackageRuntimeSmoke` creates a 32 KiB ROM-only DMG image titled `COFFEE-CI-SMOKE`, places it
  in a private temporary archive, exercises extraction, video/audio/input, and StateFile operations,
  then deletes all generated files. It reads or packages no user ROM.
- The owner-authorized Mobile Trainer/loopback-REON acceptance run added no committed ROM, save,
  private configuration, screenshot, trace, payload, or response artifact. Only sanitized facts are
  retained in [mobile-adapter-crystal-reon-validation.md](mobile-adapter-crystal-reon-validation.md);
...
were not recorded. This policy adds no copy and treats it only as an existing implementation baseline,
...
input to the synthetic machine tests, which explicitly use `BootstrapMode.SKIP`. Its hash
is recorded only to make that exclusion auditable:
`0e4ddff32fc9d1eeaae812a157dd246459b00c9e14f2f61751f661f32361e360`.

## Existing SGB-related profile assets

These files predate this policy and are exercised by the required compatibility profiles. They are
not copied into the synthetic baselines.
...
synthetic model fixture records actual Coffee GB SGB skip-boot behavior separately and does not
use these ROMs.

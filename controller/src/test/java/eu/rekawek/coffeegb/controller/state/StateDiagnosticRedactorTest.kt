...
class StateDiagnosticRedactorTest {
  ...
  fun `redacts explicit unix windows and home paths before bounding text`() {
    val root =
        Paths.get("private", "users", "alice", "save-root").toAbsolutePath().normalize()
    val input =
        "failed at $root\\Private Preview\\state.cgbstate, " +
    ...
    assertFalse(redacted.contains("Private Preview"))
  ...
  fun `redacts complete inside and outside unix paths containing spaces`() {
    val root =
        Paths.get("private", "User Data", "Coffee GB Saves").toAbsolutePath().normalize()
    val input =
